...
async function initStoredSettings() {
    ...
    try {
        ...
        // values
        document.getElementById('haldexGeneration').value = data.haldexGeneration || 1;

        document.getElementById('disengageUnderSpeedRange').value = data.disengageUnderSpeed || 0;
        document.getElementById('disengageUnderSpeed').textContent = data.disengageUnderSpeed || 0;

        document.getElementById('disengageAboveSpeedRange').value = data.disengageAboveSpeed || 0;
        document.getElementById('disengageAboveSpeed').textContent = data.disengageAboveSpeed || 0;

        document.getElementById('disableThrottleRange').value = data.disableThrottle || 0;
        document.getElementById('disableThrottle').textContent = data.disableThrottle || 0;
        ...
        // bools
        document.getElementById('disableController').checked = data.disableController || false;
        document.getElementById('isStandalone').checked = data.isStandalone || false;
        document.getElementById('tcForce5050').checked = data.tcForce5050 || false;
        document.getElementById('extButtonForce5050').checked = data.extButtonForce5050 || false;
        ...
        document.getElementById('followHandbrake').checked = data.followHandbrake || false;
        document.getElementById('invertHandbrake').checked = data.invertHandbrake || false;

        document.getElementById('broadcastOpenHaldexOverCAN').checked = data.broadcastOpenHaldexOverCAN || false;
...
async function refreshStatus() {
    try {
        ...
        document.getElementById('diagChassisCAN').textContent = chassisOk ? '✓ Healthy' : 'X Unhealthy';
        document.getElementById('diagHaldexCAN').textContent = haldexOk ? '✓ Healthy' : 'X Unhealthy';
        document.getElementById('diagFreeHeap').textContent = Math.round(data.freeHeap / 1024);
    } catch (error) {
        console.log("Status failed: " + error.message);
    }
}
...
async function saveSettings() {
    const settings = {
        haldexGeneration: parseInt(document.getElementById('haldexGeneration').value),
        disengageUnderSpeed: parseInt(document.getElementById('disengageUnderSpeedRange').value),
        disengageAboveSpeed: parseInt(document.getElementById('disengageAboveSpeedRange').value),
        disableThrottle: parseInt(document.getElementById('disableThrottleRange').value),
        ...
        tcForce5050: document.getElementById('tcForce5050').checked,
        extButtonForce5050: document.getElementById('extButtonForce5050').checked,
        ...
        broadcastOpenHaldexOverCAN: document.getElementById('broadcastOpenHaldexOverCAN').checked
    };
...
function initNavigation() {
    ...
    const disengageUnderSpeedRange = document.getElementById('disengageUnderSpeedRange');
    ...
    const disengageAboveSpeedRange = document.getElementById('disengageAboveSpeedRange');
...
function initSettings() {
    document.getElementById('saveSettings').addEventListener('click', saveSettings); // onclick, do this
    document.getElementById('saveSettingsBasic').addEventListener('click', saveSettings); // onclick, do this
}
...
function initExpertEditor() {
    ...
    for (let speed = 0; speed < arrayColumns; speed++) {
        ...
        cellSpeedAxis.setAttribute("throttle", String(0)); // make a new attribute for row
    ...
    for (let throttle = 0; throttle < arrayRows; throttle++) {
        ...
        cellThrottleAxis.setAttribute("speed", String(0)); // make a new attribuate for column
...
function refreshTrace(data) {
    ...
    const throttlePos = arrayIndex(throttle, throttleHeader);
    const speedPos = arrayIndex(speed, speedHeader);

    const cell = [...document.querySelectorAll('.map-cell')];

    for (i in cell) {
        cell[i].classList.remove('activeTrace');
    }

    cell[(throttlePos) + (speedPos * speedHeader.length)].classList.add('activeTrace');
}

// update the cell colours
function updateCellColor(cell, value) {
    cell.classList.remove('low', 'medium', 'high');
...
function confirmEdit() {
    ...
    if (throttlePos === 0 && lockTrue === "false") {
