  ...
  while (1)
  {
    ...
    while (twai_receive_v2(twai_bus_0, &rx_message_chs, 0) == ESP_OK)
    {
      ...
      if (!isGen1Standalone && !isGen2Standalone && !isGen4Standalone)
      {
        ...
        case BRAKES3_ID:
        {
          received_vehicle_speed = ((rx_message_chs.data[0] << 8) | rx_message_chs.data[1]); // confirm(!)
          isABSValid = true;
          lastABSResponse = millis();
          break;
        }

          // add tc variable in here, set to force 5050
        ...
        {
          lock_target = 0; // to reset lock target to zero - we don't know what the ECU is requesting
        }
