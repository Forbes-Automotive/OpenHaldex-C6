...
void setupUI() {
  ...
  ESPUI.addControl(Option, "75/25", "7525", Dark, int16_currentMode);
    ESPUI.addControl(Option, "Map", "Map", Dark, int16_currentMode);
  ...
  customSet_1_throttle = ESPUI.addControl(Slider, "Disable Below Throttle (%)", String(throttleArray[0]), Dark, tabCustom, generalCallback);
  ESPUI.addControl(Max, "", "100", Dark, customSet_1_throttle);
  ESPUI.setElementStyle(ESPUI.addControl(Label, "", "Throttle", None, customSet_1_throttle), clearLabelStyle);

  customSet_1_speed = ESPUI.addControl(Slider, "Disable Above Speed (kmh)", String(speedArray[0]), Dark, tabCustom, generalCallback);
  ESPUI.addControl(Max, "", "300", Dark, customSet_1_speed);
  ESPUI.setElementStyle(ESPUI.addControl(Label, "", "Speed", None, customSet_1_speed), clearLabelStyle);

  customSet_1_lock = ESPUI.addControl(Slider, "Amount of Lock (%)", String(lockArray[0]), Dark, tabCustom, generalCallback);
  ESPUI.setElementStyle(ESPUI.addControl(Label, "", "Lock %", None, customSet_1_lock), clearLabelStyle);
  ESPUI.addControl(Max, "", "100", Dark, customSet_1_lock);
...
void generalCallback(Control *sender, int type) {
  ...
  switch (tempID) {
    case 3:
      if (sender->value == "Stock") state.mode = MODE_STOCK;
      if (sender->value == "FWD") state.mode = MODE_FWD;
      if (sender->value == "5050") state.mode = MODE_5050;
      if (sender->value == "6040") state.mode = MODE_6040;
      if (sender->value == "7525") state.mode = MODE_7525;
      if (sender->value == "Custom") state.mode = MODE_CUSTOM;
      break;
    case 12:
    ...
    case 18:
      ...
      break;
    case 21:
      if (sender->value == "Gen1") haldexGeneration = 1;
      if (sender->value == "Gen2") haldexGeneration = 2;
      if (sender->value == "Gen3") haldexGeneration = 2;
      if (sender->value == "Gen4") haldexGeneration = 4;
      if (sender->value == "Gen5") haldexGeneration = 4;
      break;
    case 28:
    ...
    case 38:
      ...
      break;
    case 42:
      throttleArray[0] = sender->value.toInt();
      break;
    case 45:
      speedArray[0] = sender->value.toInt();
      break;
    case 48:
      lockArray[0] = sender->value.toInt();
      break;
  }
}
  ...
  while (1) {
    ...
    switch (state.mode) {
      ...
      case 3:
        ...
        strip.setLedColorData(led_channel, 255, 16, 240);
        break;
      case 4:
        lastMode = 4;
        ...
        break;
              case 5:
        lastMode =5 ;
        ESPUI.updateSelect(int16_currentMode, "Map");
        strip.setLedColorData(led_channel, led_brightness, led_brightness, 0);
        break;
      case 6:
        lastMode = 6;
    ...
    char haldexTemp[50];
    sprintf(haldexTemp, "Req:Act: %d:%d", int(lock_target), received_haldex_engagement);
